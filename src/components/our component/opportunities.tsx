  ...
  return (
    <section id="opportunities" className="relative bg-slate-800">
        <div className="mx-auto w-[min(1120px,92vw)] py-10 md:py-14">
          <div  >
            <img src={Opportunities} alt="" className=' relative w-full h-72 bg-center bg-cover opacity-70 rounded-xl'/>
            <div className=' absolute top-32 right-72'>
              ...
              <h2 className="mt-2 text-3xl md:text-4xl font-extrabold">
                Available land opportunities in California
              </h2>
            ...
            {[
              {
                ...
                bullets: ["Zoning & basics explained", "Location fit by preferences", "Buyer guidance"],
              },
              {
                title: "Investment Lots",
                desc: "For long-term potential and portfolio growth with clear expectations.",
                bullets: ["Investment-focused options", "Timeline support", "Next-step clarity"],
              },
              {
                title: "Mixed / Flexible",
                desc: "Not sure yet? Tell us your budget and we’ll offer the best match.",
                bullets: ["Shortlist tailored to you", "Multiple option categories", "No pressure—just clarity"],
              },
            ].map((card) => (
              <Card
                key={card.title}
                className="rounded-2xl border-white/10 bg-white/5 p-6 backdrop-blur"
              >
                <div className="text-xl font-extrabold">{card.title}</div>
                <div className=" text-sm text-white/65 leading-6">{card.desc}</div>
